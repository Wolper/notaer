<div class="content form_create">
    ...
    <article>
        ...
        if (isset($post) && $post['SendPostForm']):
            ...
            $cadastraVoo = new AdminPost;
            $cadastraVoo->ExeCreate($post);

            if ($cadastraVoo->getResult()):

                if (!empty($_FILES['gallery_covers']['tmp_name'])):
                    $sendGallery = new AdminPost;
                    $sendGallery->gbSend($_FILES['gallery_covers'], $cadastraVoo->getResult());
                endif;

                header('Location: painel.php?exe=posts/update&create=true&postid=' . $cadastraVoo->getResult());
            else:
            ...
            <div class="label_line">
                ...
                <label class="label_small">
                    ...
                    <select name="post_category">
                        ...
                        if ($readAero->getRowCount() >= 1):
                            foreach ($readAero->getResult() as $aero):
                                echo "<option disabled=\"disabled\" value=\"\"> {$aero['category_title']} </option>";
                                $readCat = new Read;
                                $readCat->ExeRead("ws_categories", "WHERE category_parent = :parent ORDER BY category_title ASC", "parent={$aero['category_id']}");
